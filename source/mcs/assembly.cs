...
namespace Mono.CSharp
{
	...
	abstract class AssemblyReferencesLoader<T> where T : class
	{
		...
		protected void LoadReferencesCore (ModuleContainer module, out T corlib_assembly, out List<Tuple<RootNamespace, T>> loaded)
		{
            ...
            //check to see if there's a copper.exe, to load that instead (for testing)
            if ( File.Exists( "Copper.exe" ) )
                copper = LoadAssemblyFile( "Copper.exe", false );
            else
                copper = LoadAssemblyFile( "Copper.dll", false );
 
            //+COPPER
            //Load the copper dll
            //T copper = LoadAssemblyFile( "Copper.dll", false );
            
            //If the dll cannot be found, try the exe file
            //if ( copper == null )
            //    copper = LoadAssemblyFile( "Copper.exe", false );

            var copper_key = Tuple.Create( module.GlobalRootNamespace, copper );

            if ( !loaded.Contains( copper_key ) )
                loaded.Add( copper_key );
