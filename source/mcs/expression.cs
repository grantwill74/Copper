...
namespace Mono.CSharp
{
    ...
    /// <summary>
    /// +COPPER
    /// Connection construct between connectables such as ports or objects.
    /// </summary>
    public class Connection : ExpressionStatement {

        private Arguments args;
        private Expression expr;

        public Connection( Arguments arguments ) {
            args = arguments;
        }

        public static bool PortsCompatible( TypeSpec a, TypeSpec b ) {
            if ( !a.IsPort )
                return false;
        ...
        protected override Expression DoResolve(ResolveContext rc) {
            if ( args.Count <= 1 )
                rc.Report.Error( 99429, "Connect statement requires at least 2 arguments." );
            
            
            var left_arg = (FieldExpr) ( (ATypeNameExpression) args[0].Expr ).Resolve( rc );
            /*var container_type = left_arg.Spec.DeclaringType;
            var left_arg_name = left_arg.Name;
            var def = left_arg.Spec.MemberDefinition;
            var left = MemberLookup( rc, false, container_type, left_arg_name, 0, 
                MemberLookupRestrictions.None, new Location() );
            */

            expr = new Invocation(
                new MemberAccess( 
                    new MemberAccess(
                        new SimpleName( "Copper", new Location() ), "Port" ), 
                    "Connect" ),
                args
            );
            eclass = ExprClass.Value;
