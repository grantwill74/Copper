...
namespace Mono.CSharp
{
	...
	public abstract class TypeContainer : MemberCore
	{
			...
			: base (parent, name, attrs)
		{
            ...
            HasPorts = false; //+COPPER
            IsComponent = false; //+COPPER
        }
        ...
        //+COPPER
        public bool HasPorts { get; set; }
        //+COPPER
        //Associated with container rather than type
        public bool IsComponent { get; set; }
        //+COPPER
        //Is this container associated with a port?
        //public bool IsPort { get { return PortType != PortTypeEnum.NotAPort; } }
        //+COPPER
        //If this container is associated with a port, does it allow 
	...
	public abstract class ClassOrStruct : TypeDefinition
	{
        ...
        //+COPPER
        //Generate code inside of class's constructors to embed ports. If 
        //there is no constructor, create a default one. 
        public void AddEmbedPortsCode() {
            var embed_statement = new Invocation(
                new SimpleName( "EmbedPorts", new Location() ),
                new Arguments( 0 )
            );

            //var embed_static_statement = new Invocation(
            //    new SimpleName( "EmbedPortsStatic", new Location() ),
            //    new Arguments( 0 )
            //);

            //we need to create a constructor if there isn't one (changed)
            /*
            if ( generated_primary_constructor != null ) {
                generated_primary_constructor.Block.AddStatement(
                    new StatementExpression( embed_statement ) );
                generated_primary_constructor.Block.IsCompilerGenerated = true;
                return;
            }
            */
            ...
            //otherwise, add Embed
            foreach ( var member in Members ) {
                if ( member.MemberName.Name == MemberName.Name ) {
                    ...
                    if ( ( member.ModFlags & Modifiers.STATIC ) != Modifiers.NONE ) { }
                        //ctor.Block.AddStatement(
                        //    new StatementExpression( embed_static_statement ) );
                    else
                        ctor.Block.AddStatement(
                            new StatementExpression( embed_statement ) );
/*
		...
		public override void Emit ()
		{
            ...
            //+COPPER
            //add the code to embed ports in the component
            if(HasPorts)
                AddEmbedPortsCode();
    ...
    /*
    //+COPPER
    public sealed class PortInterface : TypeDefinition {
        #region MODS
        const Modifiers AllowedModifiers = 
            Modifiers.PUBLIC    | 
            Modifiers.PROTECTED | 
        ...
        public PortInterface( TypeContainer parent, MemberName name, Modifiers mods, Attributes attrs ) 
            : base( parent, name, attrs, MemberKind.PortInterface ) {

            var accmods = IsTopLevel ? Modifiers.INTERNAL : Modifiers.PRIVATE;
            this.ModFlags = ModifiersExtensions.Check( AllowedModifiers, mods, accmods, name.Location, Report );
            spec = new TypeSpec( Kind, null, null, null, ModFlags );
        }
